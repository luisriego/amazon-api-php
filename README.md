...
- [Optional] Replace all the occurrences of amazon-api in the whole project with some name more meaningful for your project
...
- Create a Symfony project using the CLI (e.g. `symfony new --no-git --dir project --version="6.4.*"`). See `symfony`command info for more options
- Move the content to the root folder with `mv project/* . && mv project/.env .` 
  This is necessary since Composer won't install the project if the folder already contains data.
